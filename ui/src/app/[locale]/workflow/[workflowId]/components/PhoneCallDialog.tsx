...
export const PhoneCallDialog = ({
    ...
    const t = useTranslations("components.phoneCallDialog");
    ...
    const handleStartCall = async () => {
        ...
        try {
            ...
            if (response.error) {
                let errMsg = t("failedToInitiateCall");
                ...
                const msg = response.data && (response.data as unknown as { message: string }).message || t("callInitiatedSuccessfully");
                setCallSuccessMsg(typeof msg === "string" ? msg : JSON.stringify(msg));
            }
        } catch (err: unknown) {
            setCallError(err instanceof Error ? err.message : t("failedToInitiateCall"));
        } finally {
            setCallLoading(false);
        }
    };
    ...
    const renderConfigurationNeeded = () => (
        <>
            <DialogHeader>
                ...
                <DialogDescription>
                    {t("configureTelephonyDescription")}
                </DialogDescription>
            </DialogHeader>
            <DialogFooter>
        ...
        <>
            ...
            {telephonyConfigs.length > 0 && (
                <div className="flex flex-col gap-1.5">
                    <Label htmlFor="telephony-config">{t("telephonyConfiguration")}</Label>
                    <Select value={selectedConfigId} onValueChange={setSelectedConfigId}>
                        <SelectTrigger id="telephony-config" className="w-full">
                            <SelectValue placeholder={t("selectAConfiguration")} />
                        </SelectTrigger>
                        <SelectContent>
                            {telephonyConfigs.map((config) => (
                                <SelectItem key={config.id} value={String(config.id)}>
                                    {config.name} ({config.provider})
                                    {config.is_default_outbound ? t("defaultSuffix") : ""}
                                </SelectItem>
                            ))}
                        </SelectContent>
                    </Select>
            ...
            {selectedConfigId && (
                <div className="flex flex-col gap-1.5">
                    <Label htmlFor="from-phone-number">{t("callerIdFrom")}</Label>
                    {loadingPhoneNumbers ? (
                        ...
                        >
                            <SelectTrigger id="from-phone-number" className="w-full">
                                <SelectValue placeholder={t("selectAPhoneNumber")} />
                            </SelectTrigger>
                            <SelectContent>
                                {fromPhoneNumbers.map((phone) => (
                                    <SelectItem key={phone.id} value={String(phone.id)}>
                                        ...
                                        {phone.is_default_caller_id ? t("defaultSuffix") : ""}
                                    </SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
                        ...
                        <div className="text-xs text-muted-foreground">
                            {t("noPhoneNumbersInConfiguration")}
                        </div>
                    )}
                </div>
            )}
            ...
            >
                {sipMode ? t("usePhoneNumberInstead") : t("useSipEndpointInstead")}
            </button>
