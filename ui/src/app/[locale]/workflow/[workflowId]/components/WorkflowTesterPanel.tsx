...
export function WorkflowTesterPanel({
    ...
    const t = useTranslations("components.workflowTesterPanel");
    const auth = useAuth();
    ...
    const createVoiceRun = useCallback(async () => {
        ...
        try {
            ...
            if (response.error || !response.data?.id) {
                throw new Error(extractSdkErrorMessage(response.error, t("failedToCreateRun")));
            }
                ...
                <TabsContent value="audio" className="min-h-0 flex-1 px-4 py-4">
                    <div className="flex h-full min-h-0 flex-col gap-3">
                        {!tokenReady ? (
                            ...
                            <DisabledNotice
                                reason={authUnavailableReason ?? t("authRequiredBrowser")}
                            />
                        ) : voiceRunId ? (
                            <EmbeddedVoiceTester
                            ...
                            <>
                                ...
                                <EmptyState
                                    ...
                                    title={t("callAgentTitle")}
                                    description={t("callAgentDescription")}
                                    action={
                                        <Button
