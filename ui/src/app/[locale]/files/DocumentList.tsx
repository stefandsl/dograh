...
export default function DocumentList({ refreshTrigger }: DocumentListProps) {
  const t = useTranslations('components.documentList');
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {doc.retrieval_mode === 'full_document' ? (
                      <Badge variant="outline" className="text-xs">{t('retrievalFullDocument')}</Badge>
                    ) : (
                      <Badge variant="outline" className="text-xs">{t('retrievalChunked')}</Badge>
                    )}
                  </div>
                  <div className="flex items-center gap-4 text-sm text-muted-foreground">
                    <span>{formatFileSize(doc.file_size_bytes)}</span>
                    {doc.processing_status === 'completed' && doc.retrieval_mode !== 'full_document' && (
                      <span>{t('chunksCount', { count: doc.total_chunks })}</span>
                    )}
                    <span>{formatDate(doc.created_at)}</span>
                  </div>
                  {doc.processing_error && (
                    <p className="text-xs text-destructive mt-1">
                      {t('errorPrefix', { error: doc.processing_error })}
                    </p>
                  )}
