...
export default function RecordingsList({ refreshKey }: { refreshKey?: number }) {
    const t = useTranslations("components.recordingsList");
    ...
    const saveRecordingId = async (rec: RecordingResponseSchema) => {
        ...
        if (!newId) {
            setEditError(t("idEmptyError"));
            return;
        }
        if (!/^[a-zA-Z0-9_-]+$/.test(newId)) {
            setEditError(t("idInvalidError"));
            return;
        }
        ...
        try {
            ...
            if (response.error) {
                ...
                throw new Error(errData?.detail || t("updateError"));
            }

            toast.success(t("updateSuccess", { newId }));
            cancelEditing();
            fetchRecordings();
        } catch (err) {
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="text-sm text-muted-foreground">
                {t("resultsCount", { count: filteredRecordings.length })}
