...
async def _config_preflight_response(token: str, origin: str) -> Response:
    ...
    # Internal tokens (Telegram WebApp) bypass origin validation — keep the
    # preflight consistent with the GET handler (see _origin_check_required).
...
async def _turn_credentials_preflight_response(
    ...
    # Internal tokens (Telegram WebApp) bypass origin validation — keep the
    # preflight consistent with the TURN-credentials handler.
    if _origin_check_required(embed_token) and not validate_origin(
        origin, embed_token.allowed_domains or []
    ):
        return Response(status_code=403)
