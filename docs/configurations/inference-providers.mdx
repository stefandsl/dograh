...
description: "Voice Agents need AI Models to work, like LLM (Large Language Model), TTS (Voice) and STT (Transcriber). You can use any of your faviourite providers with Bellerophone Platform to run your Voice Agent."
---

## How Model Configuration Works

Bellerophone uses a **two-level configuration system** for AI models:
...
The global configuration is the default model setup shared across all your agents. Bellerophone ships with its own models by default — when you sign up on https://app.dograh.com or set up the platform on your self-hosted infrastructure, you get some Bellerophone model credits to start with.
...
Select a provider from the dropdown and configure the API key, model, and any provider-specific settings. For Bellerophone's own models, see [Service Keys](api-keys) for instructions on creating Service Keys.
...
Bellerophone supports Gemini 3.1 Live as a **Realtime** provider. The default model is `gemini-3.1-flash-live-preview`.
...
To use Gemini 3.1 Live with Bellerophone, you need a Google Gemini API key. Follow these steps:
...
<Note>
  The Gemini API key is different from a Google Cloud service account key. You specifically need a **Gemini API key** from Google AI Studio for use with Bellerophone.
</Note>

### Configuring Gemini 3.1 Live in Bellerophone

1. Go to **Model Configurations** in your Bellerophone dashboard (`https://app.dograh.com/model-configurations` for hosted or `http://localhost:3010/model-configurations` for local).
...
<Note>
  Always pick **JSON**, not P12. The Vertex AI client libraries used by Bellerophone only accept service-account JSON keys; P12 is a legacy format retained for older Google Workspace integrations.
</Note>

### Configuring Vertex AI Realtime in Bellerophone

1. Go to **Model Configurations** in your Bellerophone dashboard.
...
<Note>
  Paste the whole JSON file into the **Credentials** field — including `private_key`, `client_email`, and all other entries. Don't try to extract individual fields. If `Credentials` is left blank, Bellerophone falls back to **Application Default Credentials (ADC)** from the host environment, which is useful when running Bellerophone on a GCP VM or GKE pod with an attached service account.
</Note>
