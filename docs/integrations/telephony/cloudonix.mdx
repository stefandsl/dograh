...
description: "Configure Cloudonix for voice communication in Bellerophone AI"
---

## Overview

Cloudonix is a cloud communications platform that enables Bellerophone voice agents to connect to SIP communications
...
- A Cloudonix **Voice Application** on that domain (optional — leave the field blank in Bellerophone and we'll auto-create one for you on save, with the application `url` pre-set)
- A Cloudonix outbound voice trunk service provider connection
- Bellerophone AI instance running and accessible

## Video Tutorial

Watch this step-by-step guide to set up Cloudonix with Bellerophone AI:

<iframe
  className="w-full aspect-video rounded-xl"
  src="https://www.youtube.com/embed/qLKX0F99jpU?si=tA3zwCsNRKe-WCI1"
  title="Bellerophone Cloudonix Setup"
...
3. (Optional) Navigate to your domain's **Applications** and create (or open) the application you'll use with Bellerophone, then copy its **Application Name**. Skip this if you want Bellerophone to auto-create the Voice Application for you on save.

### Step 2: Configure in Bellerophone AI
...
4. Enter your credentials:
   - Bearer Token
   - Domain ID
   - Application Name — *optional*. Leave blank and Bellerophone will auto-create a Voice Application on save (with the application `url` and CXML runtime already configured) and store its name on this configuration.
...
6. Open the configuration you just created and add at least one **phone number** (with country code in E.164 format, e.g. `+1234567890`). The default caller ID is used for outbound calls.
   ...
   <Note>
     If Bellerophone auto-created the Voice Application for you, you still need to
...
Cloudonix routes inbound calls per **Voice Application** — the webhook URL is set once on the application, and applies to every DNID bound to it. **When you save an inbound workflow on a phone number, Bellerophone automatically pushes the webhook URL to your Voice Application's `url`** (provided the credentials are correct), so you don't need to set the webhook by hand. If Bellerophone auto-created the application during configuration save, the `url` is already set and this push is a no-op — you only need to bind your DNIDs to the auto-created application.
...
2. Create a new application (or open the existing one whose name you configured in Bellerophone) with these settings:
...
4. Save
   ...
   <Note>
     The Application URL is what Bellerophone's auto-push updates in Step 3 — you
     ...
     Either works. Self-hosted Bellerophone deployments use their backend domain
   ...
   <img
     ...
     alt="Cloudonix application form showing Remote Application Resource type, Cloudonix CXML runtime, the Bellerophone inbound URL, POST method, and a DNID number entry"
   />
   <img
     className="hidden dark:block"
     src="/images/cloudonix-inbound-config-application.png"
...
### Step 3: Assign an Inbound Workflow to the Phone Number in Bellerophone
...
2. Open the application whose name you configured in Bellerophone
3. Confirm:
   - **URL** is set to: `https://api.dograh.com/api/v1/telephony/inbound/run`
   - **Method** is `POST`

   <Note>
     Bellerophone pushed this URL automatically when you saved the inbound workflow
     in Step 3. The same URL is shared across every DNID bound to that
     application — Bellerophone routes each inbound call to the right agent based
     on the called number's inbound workflow assignment. If the field is
     empty, shows a different URL, or Bellerophone surfaced a sync warning on
     save, the auto-push failed — most often because the Bearer Token,
     Domain ID, or Application Name in Bellerophone is incorrect. Paste the URL
     into the field yourself, set the method to `POST`, and save. On
     self-hosted Bellerophone, replace `api.dograh.com` with your backend domain.
   </Note>
...
- Ensure your Bellerophone AI instance is publicly accessible
...
2. Verify your Bellerophone AI voice agent answers and responds
3. Check call logs in both Bellerophone AI dashboard and Cloudonix Console
...
<AccordionGroup>
  ...
  <Accordion title="Inbound calls not reaching voice agent">
    ...
    configured in Bellerophone - Confirm the called number exists in your Bellerophone
    telephony configuration and has an **Inbound workflow** assigned -
    After assigning the inbound workflow, confirm Bellerophone successfully updated
  ...
  <Accordion title="Voice agent doesn't respond to inbound calls">
    ...
    /telephony-configurations - Verify the Bearer Token in Bellerophone matches the
...
- Use a dedicated Voice Application for Bellerophone so the shared `url` doesn't conflict with other systems
