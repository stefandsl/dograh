...
description: "Configure inbound calling for Twilio, Vonage, Plivo, Telnyx, Cloudonix, Vobiz, and Asterisk ARI providers in Bellerophone AI"
---

## Overview

Bellerophone AI supports inbound calling across all supported telephony providers. When someone calls your configured phone number, your voice agent automatically answers and handles the interaction. Inbound and outbound calls share the same telephony configuration.
...
Bellerophone exposes a **single inbound webhook URL** for the whole org:
...
3. **Org & Agent Resolved**: Bellerophone identifies the org from the webhook's account credentials, then looks up which agent should answer based on the called number's **Inbound workflow** assignment in your telephony configuration
...
You don't construct per-workflow URLs anymore — the routing is done by the called number's inbound workflow assignment in Bellerophone.

## Configuration in Bellerophone AI
...
This is what tells Bellerophone which agent to run when a call comes in for that number.

### Step 3: Point Your Provider at `/inbound/run`

Each provider needs its inbound webhook (Twilio Voice URL / Vonage Answer URL / etc.) set to the global Bellerophone endpoint:
...
<Note>
If you're self-hosting Bellerophone, replace `api.dograh.com` with your own domain.
</Note>
...
After completing both the Bellerophone AI configuration and provider-specific setup:
...
3. **Check Logs**: Monitor call logs in both Bellerophone AI and your provider's dashboard
...
<AccordionGroup>
  ...
  <Accordion title="Provider mismatch error">
    ...
    - Ensure the phone number is configured for the same provider in both Bellerophone AI and your telephony console
  </Accordion>
  
  <Accordion title="Workflow not found error">
    - **Error**: "Workflow not found"
    - **Solution**: Open the called number in **/telephony-configurations** and confirm an **Inbound workflow** is assigned. Without an assignment, Bellerophone has no agent to route the call to.
  ...
  <Accordion title="Account validation failed">
    ...
    - **Solution**: Verify your account SID/credentials configuration in Bellerophone AI matches your telephony provider settings
  ...
  <Accordion title="Signature validation failed">
    ...
    - **Solution**: Verify your auth token configuration in Bellerophone AI matches your provider settings
  ...
  <Accordion title="Poor audio quality on inbound calls">
    - Check network bandwidth and latency from provider to Bellerophone AI
...
- **Monitoring**: Monitor both Bellerophone AI logs and provider dashboards for call analytics
